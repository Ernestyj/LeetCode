...
public:
    NumMatrix(vector<vector<int>>& matrix) {
        ...
        dp.resize(m+1, vector<int>(n+1, 0));    //多分配1个可以免去初始化第一行和第一列的dp值
        for (int i = 1; i <= m; ++i) {
            for (int j = 1; j <= n; ++j) {
...
//        int iMin = min(row1, row2), iMax = max(row1, row2); //You may assume that row1 ≤ row2 and col1 ≤ col2.
//        int jMin = min(col1, col2), jMax = max(col1, col2);
        return dp[row2+1][col2+1] - dp[row2+1][col1] - dp[row1][col2+1] + dp[row1][col1]; //TODO 注意容易出错
    }
};

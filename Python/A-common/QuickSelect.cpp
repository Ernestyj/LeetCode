...
//                }
//            }// 最后退出的情况应该是右指针在左指针左边一格
//            if(left<j && k<=j) return quickSelect(arr, k, left, j);   // 这时如果右指针还大于等于k，说明kth在左半边
//            if(i<right && k>=i) return quickSelect(arr, k, i, right); // 这时如果左指针还小于等于k，说明kth在右半边
//            return arr[k];
//        }

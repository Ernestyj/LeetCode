...
public:
    int kthSmallest(TreeNode* root, int k) {
        ...
        if(k<=lCount) return kthSmallest(root->left, k);    // 在左子树
        else if(k==lCount+1) return root->val;  // 1 is counted as current node
        else return kthSmallest(root->right, k-lCount-1);   // 在右子树
    }

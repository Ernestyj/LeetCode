...
public:
    string getPermutation(int n, int k) {
        string res, orig = "123456789";
        vector<int> fact(n, 1);
        for (int i = 1; i < n; ++i) fact[i] = fact[i-1]*i;
        --k;    //k转换为从0开始
        for (int i = n; i >= 1; --i) {  //计算Kth全排列串中第i位(1=<i<=n)数字
            int index = k/fact[i-1];    //得到第i位数字字符
            k = k%fact[i-1];            //更新k
            res.push_back(orig[index]);
            orig.erase(index, 1);       //每确定一位数字，“原字符串”应移除该数字
        }
        return res;
    }
};
